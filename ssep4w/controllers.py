...
def index():
    user = auth.get_user()
    message = T(
        "Hello {first_name}".format(**user) if user else "./py4web.py run apps --watch=off -s wsgirefThreadingServer"
    )
    ...
    menu = DIV(
        ...
        DIV(
            DIV( "+ lastEventId, reconnect, redis, pydal"),

             A(
                "ltask",
                _role="button",
                _href=URL( 'ltask/ltask' ),
              ),
