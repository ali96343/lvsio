...
try:
    from .left_menu import (l_menu, b_menu, t_menu, u_menu )
except ImportError:
    l_menu = []
...
@action("mi7")
@action.uses("mi7.html", auth, T, Inject(l_menu=l_menu))
def mi7():
    user = auth.get_user()
    message = T("mi7 Hello {first_name}".format(**user) if user else "mi7 Hello")
    actions = {"allowed_actions": auth.param.allowed_actions}
    return dict(
        message=message,
        actions=actions,
