...
l_menu = [
    ...
    ( "adm1", False, "#", [
            ctrl_item('db_tables'),
            nav_item("manager", "adm1/find_tag/manager"),
            ctrl_item("adm1/find_tag/dancer"),
            ctrl_item("adm1/find_tag/teacher"),
        ],
    ),
...
def ulli_menu( x_menu=l_menu  ):
    ulli = []
    ulli.append('<div class="as-talk-details">')
    ulli.append('<ul>')
    for _item in x_menu or []:
        for _ii in _item[3]:
            ulli.append(f'<li>{_item[0]}</li>')
            ulli.append(f'<li>{_ii[0]}</li>')
            ulli.append(f'<li><a role="button" href="{_ii[2]}">go</a></li>')

    ulli.append('</ul>')
    ulli.append('</div>')
    return ''.join( ulli  ) 
    
u_menu = ulli_menu( x_menu=l_menu  )
